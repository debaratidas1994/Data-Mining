	...
	def DBSCAN(self):  
		for i in range(len(self.DB)):  
			...
			if (not p_tmp.visited):  
				p_tmp.visited = True 
				self.visit_cnt += 1 
				NeighborPts = self.epsilon_neighbourhood(p_tmp)  #check if it satisfies epsilon
				if(len(NeighborPts) < self.MinPts):  
					...
					self.check_neighbourhood(p_tmp, NeighborPts) #check if it satisfies minPoints
...
if __name__=='__main__': 
   ...
   # Writing new image
	...
	print("Noise points:",len(dbScan.noise))
	...
	template="{0:15}|{1:30}|{2:40}"
	print(template.format("Cluster","Cluster Size","Avg Cluster Tuple"))
	print()
	k=0
	for i in cluster_tmp:
		...
		print(template.format(cluster_tmp.index(i)+1,len(i),dbScan.clusterCentroids[k]))
		k+=1
	"""
	for i in dbScan.clusterCentroids :
		print i
		print()
	"""